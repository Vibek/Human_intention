...
// ---------- FOR THE RIGHT HAND ---------------------------------------------------------------
			...
			listener.lookupTransform("/right_hand_1", "/openni_depth_frame", ros::Time(0), transform1);
...
// --------------------- FOR THE LEFT HAND ------------------------------------------------------			
			...
			listener.lookupTransform("/left_hand_1", "/openni_depth_frame", ros::Time(0), transform2);
...
// --------------------- FOR THE HEAD ------------------------------------------------------			
			...
			listener.lookupTransform("/head_1", "/openni_depth_frame", ros::Time(0), transform2);
...
//putting the values in vector to save
    ...
    ofstream x_file;
    vector<double>::iterator i = right_hand_x.begin();
    ...
    x_file.open ("hand_update_left.csv", ios::app); //the flag ios::app makes sure that the control starts at the end of already existing file.
		x_file << 2;
		x_file << ",";

    for (vector<double>::iterator l = head_x.begin(); l != head_x.end();++l){
		//cout << *l << endl;
        /*      x_file << *i;
		x_file << ",";
		i++;
