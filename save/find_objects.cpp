...
bool ObjectsFinderFeature2D::readObjectsFile(char *filename)
	{
	  ...
	  if (myReadFile.is_open()) {
	   while (!myReadFile.eof()) 
	   {
	      ...
	      cv::Mat img_object = cv::imread( img_name, CV_LOAD_IMAGE_GRAYSCALE );
	      if(!img_object.data)
	        return false;
        ...
        */
	      ...
	      resize(img_object,img_object, cv::Size((float)scene_size.width/2.0, (float)scene_size.width/2.0* ((float)img_object.rows/(float)img_object.cols)));
	      char s[20];
...
bool ObjectsFinderSURF::detectAndCompute(cv::Mat &img, vector<cv::KeyPoint> &kp, cv::Mat &desc)
{
...
bool ObjectsFinderSURF::isSquared( vector<cv::Point> points)
  {
...
cv::Mat ObjectsFinderSURF::detectObjects(cv::Mat frame)
{
  	//resize(frame,frame,scene_size);
    cv::Mat img_scene(frame.size(),CV_8UC1);
    ...
    cv::Mat img_matches=frame.clone();
...
//      printf("  Min dist : %f \n", min_dist );
      ...
      if(obj.size()>4 && scene.size()>4)
      {
        ...
        vector<cv::Point> obj_corners_p(4);
        ...
        vector<cv::Point> hull;  // Convex hull points 
        ...
        approxPolyDP(cv::Mat(hull), contour, 1, true);
//        cout << Mat(contour) << endl;

        float cont=(cv::contourArea(cv::Mat(contour)));
//        printf("Area: %f",cont);
        ...
        if(current_only)
        {
          line( img_matches, scene_corners[0] + cv::Point2f( img_objects[n].cols, 0), scene_corners[1] + cv::Point2f( img_objects[n].cols, 0), cv::Scalar(255, 0, 0), 4 );
         line( img_matches, scene_corners[1] + cv::Point2f( img_objects[n].cols, 0), scene_corners[2] + cv::Point2f( img_objects[n].cols, 0), cv::Scalar(255, 0, 0), 4 );
         line( img_matches, scene_corners[2] + cv::Point2f( img_objects[n].cols, 0), scene_corners[3] + cv::Point2f( img_objects[n].cols, 0), cv::Scalar( 255, 0, 0), 4 );
         line( img_matches, scene_corners[3] + cv::Point2f( img_objects[n].cols, 0), scene_corners[0] + cv::Point2f( img_objects[n].cols, 0), cv::Scalar( 255, 0, 0), 4 );
          putText(img_matches, object_names[n], cv::Point(scene_corners[0].x+img_objects[n].cols, scene_corners[0].y-10), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255,0,0), 2);        
        }
        else
        {
          ...
          if(n==current_object)
          {  
            line( img_matches, scene_corners[0], scene_corners[1], cv::Scalar(0, 0, 255), 4 );
            line( img_matches, scene_corners[1], scene_corners[2], cv::Scalar( 0, 0, 255), 4 );
            line( img_matches, scene_corners[2], scene_corners[3], cv::Scalar( 0, 0, 255), 4 );
            line( img_matches, scene_corners[3], scene_corners[0], cv::Scalar( 0, 0, 255), 4 );

			putText(img_matches, object_names[n], cv::Point(scene_corners[0].x, scene_corners[0].y-10), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0,0,255), 2);
			putText(img_matches, s, cv::Point(scene_corners[0].x, scene_corners[0].y+10), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0,0,255), 2);
          ...
          {  
            ...
            putText(img_matches, object_names[n], cv::Point(scene_corners[0].x, scene_corners[0].y-10), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0,255,0), 2);
            putText(img_matches, s, cv::Point(scene_corners[0].x, scene_corners[0].y+10), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0,255,0), 2);
...
bool ObjectsFinderORB::detectAndCompute(cv::Mat &img, vector<cv::KeyPoint> &kp, cv::Mat &desc)
{
  if (orbDetector) {
...
cv::Mat ObjectsFinderORB::detectObjects(cv::Mat frame)
{
    //resize(frame,frame,scene_size);
    cv::Mat img_scene(frame.size(),CV_8UC1);
    cvtColor(frame,img_scene,CV_BGR2GRAY);
...
void ObjectsFinderCMB::init(int width, int height)
{
  ...
  CMBdetector = new cv::DynamicAdaptedFeatureDetector (new cv::FastAdjuster(10, true), 5000, 10000, 10);
...
bool ObjectsFinderCMB::detectAndCompute(cv::Mat &img, vector<cv::KeyPoint> &kp, cv::Mat &desc)
{
...
cv::Mat ObjectsFinderCMB::detectObjects(cv::Mat frame)
{
    //resize(frame,frame,scene_size);
    cv::Mat img_scene(frame.size(),CV_8UC1);
    cvtColor(frame,img_scene,CV_BGR2GRAY);
