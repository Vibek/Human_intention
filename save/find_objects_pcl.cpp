...
void ObjectFinderPCL::processDepth(const XnDepthPixel* pDepth)
{
       ...
       //memcpy( pdepth.data, pDepth, cols*rows*sizeof(XnDepthPixel) );
   
    XnUInt64 noSampleValue = 0, shadowValue = 0;
    //cv::Mat badMask = (pdepth == noSampleValue) | (pdepth == shadowValue) | (pdepth == 0);
   
       // mask the pixels with invalid depth
   // depth.setTo( cv::Scalar::all( CvCapture_OpenNI::INVALID_PIXEL_VAL ), badMask );
}

void ObjectFinderPCL::processPoint(const sensor_msgs::PointCloud2ConstPtr& msg) {
